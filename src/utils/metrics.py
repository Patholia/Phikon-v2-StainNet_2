...
def compute_uni_fid(generated, real):
    """Frechet distance in ViT-L/16 pathology feature space.

    Uses CLS token features from Phikon-v2 (Owkin, 2024) — an open-access
    drop-in replacement for UNI with identical ViT-L/16 architecture and
    1024-dim embeddings. Pathology-specific alternative to Inception FID.

    Args:
        generated, real: [N, 3, H, W] in [-1, 1]

    Returns:
        float: UNI-FID value (computed with Phikon-v2 features)
    """
    from transformers import AutoModel
    import torchvision.transforms as transforms
    from scipy.linalg import sqrtm

    uni_model = AutoModel.from_pretrained("owkin/phikon-v2")
    ...
    def extract_cls_features(images):
        ...
        for i in range(0, len(images), 16):
            ...
            with torch.no_grad():
                out = uni_model(pixel_values=batch_norm.cuda()).last_hidden_state
...
def compute_downstream_metrics(generated, real, labels, train_ihc_dir):
    ...
    # Load Phikon-v2 (open-access ViT-L/16 pathology foundation model, drop-in for UNI)
    print("  Loading Phikon-v2 model for downstream evaluation...")
    uni_model = AutoModel.from_pretrained("owkin/phikon-v2")
    uni_model = uni_model.cuda().eval()
    ...
    def extract_features_from_dir(img_dir):
        ...
        for fn in filenames:
            ...
            if len(batch_imgs) == 16:
                ...
                with torch.no_grad():
                    out = uni_model(pixel_values=batch.cuda()).last_hidden_state
                    feats.append(out[:, 0, :].cpu())
