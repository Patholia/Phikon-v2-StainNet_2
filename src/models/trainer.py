...
class UNIStainNetTrainer(pl.LightningModule):
    ...
    def _load_uni_model(self):
        """Lazily load Phikon-v2 ViT-L/16 for on-the-fly feature extraction.

        Phikon-v2 (owkin/phikon-v2) is used as an open-access drop-in replacement
        for UNI: same ViT-L/16 architecture, same patch size (16), same embedding
        dim (1024), same output shape [B, 197, 1024]. Loaded via HuggingFace
        transformers (public, no gating) instead of timm.
        """
        if self._uni_model is None:
            from transformers import AutoModel
            self._uni_model = AutoModel.from_pretrained("owkin/phikon-v2")
            ...
            print(f"Phikon-v2 loaded for on-the-fly extraction: {n_params:,} params")
    ...
    def _extract_uni_from_sub_crops(self, uni_sub_crops):
        ...
        # Batched forward: [B, 16, 3, 224, 224] -> [B*16, 3, 224, 224]
        all_crops = uni_sub_crops.reshape(B * 16, 3, 224, 224).to(self.device)
        # Phikon-v2 uses HuggingFace transformers API instead of timm.forward_features.
        # Output shape is identical to UNI: [B*16, 197, 1024] (1 CLS + 196 patch tokens).
        outputs = uni_model(pixel_values=all_crops)
        all_feats = outputs.last_hidden_state  # [B*16, 197, 1024]
