...
def main():
    # A30/A100 Tensor Core'larından tam fayda almak için TF32 matmul'u medium'a al.
    # Doğrulukta gözle görülür kayıp yok, GEMM'lerde anlamlı hızlanma.
    ...
    trainer = pl.Trainer(
        ...
        accelerator='gpu',
        devices=1,
        precision='bf16-mixed',          # A30 / A100 / H100 için karışık bf16
    ...
    # PyTorch Lightning, args.resume_from None ise sıfırdan başlar; bir checkpoint
    # yolu verildiyse model + optimizer + scheduler + epoch sayacı hepsi yüklenir
    # ve eğitim kaldığı epoch'tan kesintisiz devam eder.
    # Bu sayede --max_epochs'u kademeli artırdığımızda (20 → 40 → 60 → 80 → 100)
    # her chunk sadece kalan epoch'ları yapar.
