...
# Patholia — PHASE 2 · BATCH 2.6  (HoVerNext train chunk 5 / 7)
# Hedef: ~120K → 150K
set -e

TARGET_STEPS=150000
NEXT_BATCH_NAME="batch2.7.sh"

echo "=========================================================="
echo "  BATCH 2.6  —  HoVerNext train chunk 5/7 (→ ${TARGET_STEPS} step)"
...
ls "$EXP_DIR"/*.pth 1>/dev/null 2>&1 || { echo "[batch2.6] HATA: önceki ckpt yok"; exit 1; }
LAST_CKPT=$(ls -t "$EXP_DIR"/*.pth | head -1)
echo "[batch2.6] Resume: $LAST_CKPT"

CONFIG_FILE="$WORK/checkpoints/hovernext_config_b2_6.toml"
...
mkdir -p "$LOGS_DIR"
cp -a "$HOME/patholia_b2_6_${SLURM_JOB_ID}".{out,err} "$LOGS_DIR/" 2>/dev/null
