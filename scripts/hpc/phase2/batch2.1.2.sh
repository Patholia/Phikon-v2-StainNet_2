#!/bin/bash
#SBATCH --job-name=patholia_b2_1_2_PR
...
# Patholia — PHASE 2 · BATCH 2.1.2  (PR conversion)
set -e

MARKER="PR"
NEXT_BATCH_NAME="batch2.1.3.sh"

echo "=========================================================="
echo "  BATCH 2.1.2  —  $MARKER conversion"
...
if [ -f "$OUTPUT_DIR/$MARKER/images.npy" ] && [ -f "$OUTPUT_DIR/$MARKER/labels.npy" ]; then
    echo "[batch2.1.2] $MARKER zaten dönüşmüş — atlanıyor."
else
    python "$WORK/code/veri_boyut/pannuke_to_ihc.py" \
        ...
        --device     cuda \
        --cache_dir  "$HF_HOME"
    [ -f "$OUTPUT_DIR/$MARKER/images.npy" ] || { echo "[batch2.1.2] HATA: $MARKER üretilmedi"; exit 1; }
    echo "[batch2.1.2] $MARKER OK: $(ls -lh $OUTPUT_DIR/$MARKER/*.npy)"
fi

LOGS_DIR="$WORK/datasets/pannuke_ihc/logs"
mkdir -p "$LOGS_DIR"
cp -a "$HOME/patholia_b2_1_2_${MARKER}_${SLURM_JOB_ID}".{out,err} "$LOGS_DIR/" 2>/dev/null
...
echo "Finished batch 2.1.2 ($MARKER) : $(date)"
