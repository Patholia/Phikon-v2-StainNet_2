...
# Patholia — PHASE 2 · BATCH 2.3  (HoVerNext train chunk 2 / 7)
# Hedef: ~30K → 60K
set -e

TARGET_STEPS=60000
NEXT_BATCH_NAME="batch2.4.sh"

echo "=========================================================="
echo "  BATCH 2.3  —  HoVerNext train chunk 2/7 (→ ${TARGET_STEPS} step)"
...
ls "$EXP_DIR"/*.pth 1>/dev/null 2>&1 || { echo "[batch2.3] HATA: önceki ckpt yok"; exit 1; }
LAST_CKPT=$(ls -t "$EXP_DIR"/*.pth | head -1)
echo "[batch2.3] Resume: $LAST_CKPT"

CONFIG_FILE="$WORK/checkpoints/hovernext_config_b2_3.toml"
...
mkdir -p "$LOGS_DIR"
cp -a "$HOME/patholia_b2_3_${SLURM_JOB_ID}".{out,err} "$LOGS_DIR/" 2>/dev/null
