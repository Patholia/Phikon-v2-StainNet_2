...
# Patholia — PHASE 2 · BATCH 2.7  (HoVerNext train chunk 6 / 7)
# Hedef: ~150K → 180K
set -e

TARGET_STEPS=180000
NEXT_BATCH_NAME="batch2.8.sh"

echo "=========================================================="
echo "  BATCH 2.7  —  HoVerNext train chunk 6/7 (→ ${TARGET_STEPS} step)"
...
ls "$EXP_DIR"/*.pth 1>/dev/null 2>&1 || { echo "[batch2.7] HATA: önceki ckpt yok"; exit 1; }
LAST_CKPT=$(ls -t "$EXP_DIR"/*.pth | head -1)
echo "[batch2.7] Resume: $LAST_CKPT"

CONFIG_FILE="$WORK/checkpoints/hovernext_config_b2_7.toml"
...
mkdir -p "$LOGS_DIR"
cp -a "$HOME/patholia_b2_7_${SLURM_JOB_ID}".{out,err} "$LOGS_DIR/" 2>/dev/null
