...
# Patholia — PHASE 2 · BATCH 2.5  (HoVerNext train chunk 4 / 7)
# Hedef: ~90K → 120K
set -e

TARGET_STEPS=120000
NEXT_BATCH_NAME="batch2.6.sh"

echo "=========================================================="
echo "  BATCH 2.5  —  HoVerNext train chunk 4/7 (→ ${TARGET_STEPS} step)"
...
ls "$EXP_DIR"/*.pth 1>/dev/null 2>&1 || { echo "[batch2.5] HATA: önceki ckpt yok"; exit 1; }
LAST_CKPT=$(ls -t "$EXP_DIR"/*.pth | head -1)
echo "[batch2.5] Resume: $LAST_CKPT"

CONFIG_FILE="$WORK/checkpoints/hovernext_config_b2_5.toml"
...
mkdir -p "$LOGS_DIR"
cp -a "$HOME/patholia_b2_5_${SLURM_JOB_ID}".{out,err} "$LOGS_DIR/" 2>/dev/null
